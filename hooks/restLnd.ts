...
class LndApi {
  ...
  constructor(host: string = 'localhost', port: string = '8080', pathPrefix: string = '/v1') {
    this.host = host;
    this.port = port;
  ...
  url = (path: string, queryParams: ?string) => {
    ...
    let finalPath = 'https://' + this.host + ':' + this.port + this.pathPrefix + path;
    if (queryParams) {
  ...
  addPeers = async (pubkey: string, host: string, perm: boolean = false) => {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        // overcome that.
        const peerIncludes = (await this.peers())['peers'].map((a) => a.pub_key).includes(pubkey);
  ...
  openChannel = async (channelRequest: object) => {
    ...
    if (channelRequest.node_pubkey_string.includes('@')) {
      channelRequest.node_pubkey_string = channelRequest.node_pubkey_string.split('@')[0];
    }
